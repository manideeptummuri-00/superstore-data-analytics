...
Project status: SQL phase complete. Python (EDA) and Tableau (executive dashboard) phases in progress — see Future Improvements.
...
Scope: Cleaned dataset is scoped to United States only (9,994 rows) — see Data Cleaning Process for why
...
Data Cleaning Process
Verified data quality before cleaning — zero NULLs across all columns, zero duplicate Row IDs, zero fully-duplicated rows, zero logical date violations (ship date never precedes order date).
Scoping decision: the raw dataset contains 200 Canadian rows, but the region field (Central/East/South/West) is a US-only geographic hierarchy. Mixing Canadian rows in would corrupt every regional rollup, so the cleaned table is scoped to United States only (9,994 rows). This is a documented decision, not a silent drop — the verification query is in 02_data_cleaning.sql.
Type correction: order_date/ship_date converted from text to DATE; postal_code kept as VARCHAR (it's an identifier, not a number).
Indexing: added on order_date, customer_id, category+sub_category, region, and order_id to support the query patterns used across the 30 business questions.
Feature Engineering
...
Business Questions Solved
The full set of 30 questions lives in sql/03_business_questions.sql, organized into 8 technique categories. Representative examples:

Which sub-categories are actually losing money overall? (GROUP BY + HAVING)
Does discounting hurt profit — how does profit differ across discount bands? (CASE)
Which customers have above-average total sales? (Subquery)
What is month-over-month sales growth? (CTE + LAG)
For each region, what's the best-performing category by profit? (CTE + ROW_NUMBER)
Rank the top 5 products by sales within each category. (Window function: RANK)
What is the running cumulative total of sales by month? (Window function: running SUM)
What percentage of total company profit does each category contribute? (CTE + scalar subquery)
Show each order's sales alongside that customer's lifetime value. (INNER JOIN)
Compare each region's average order profit to the company-wide average. (Derived-table join)
Project Workflow
Load raw CSV into a staging table with LOAD DATA INFILE (01_database_setup.sql)
Run data quality verification queries and document findings (02_data_cleaning.sql)
Build a cleaned, typed, feature-engineered analysis table with indexes
Answer 30 business questions spanning every major SQL technique (03_business_questions.sql)
Wrap the most reusable queries into views for downstream BI consumption
SQL Views Created
...
Author
Manideep TUmmuri B.Tech Mining Engineering, IIT (ISM) Dhanbad
